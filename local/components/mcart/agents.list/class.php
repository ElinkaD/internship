...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
         ...
         */
        if(!isset($arParams['PAGE_SIZE']) || intval($arParams['PAGE_SIZE']) <= 0){
            $arParams['PAGE_SIZE'] = 5;
        }
        if(!isset($arParams['CACHE_TIME']) || intval($arParams['CACHE_TIME']) < 0){
            $arParams['CACHE_TIME'] = 3600; //час
        }
    ...
    final public function executeComponent(): void
    {
        if (empty($this->arParams["HLBLOCK_TNAME"])) {
             ...
             */
            $this->addError(
                new Error(Loc::getMessage('MCART_AGENTS_LIST_NOT_HLBLOCK_TNAME'), 400)
            );
        }
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
             ...
             */
            $enumResult = \CUserFieldEnum::GetList([], ['USER_FIELD_ID' => $fieldID]);
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)
            ->setPageSize($this->arParams['PAGE_SIZE']) //Нужно передать параметр Количество элементов из мввсива $this->arParams
            ->initFromUri();
        ...
        $rsAgents = $entity::getList([
            /*
                ...
                */
            'select' => ['ID', 'UF_NAME','UF_ACTIVE','UF_EMAIL','UF_PHONE','UF_TYPE_ACTIVITY', 'UF_AVA'],
            'filter' => ['=UF_ACTIVE' => 1],
            'count_total' => true,
            'limit' => $nav->getLimit(),
        ...
        while ($arAgent = $rsAgents->fetch()) {
             ...
             */
            if(isset($arAgent['UF_TYPE_ACTIVITY'])){
                $arAgent['UF_TYPE_ACTIVITY_NAME'] = $arTypeAgents[$arAgent['UF_TYPE_ACTIVITY']];
            }

            if($arAgent['UF_AVA']){
                $arAgent['UF_AVA_PATH'] = \CFile::GetPath($arAgent['UF_AVA']);
            }
    ...
    public function clickStarAction($agentID){
        ...
        // Получаем текущие избранные агенты пользователя
        $value = CUserOptions::GetOption($category, $name); // массив ID элементов, которые пользователь добавил в избраное
        if (!is_array($value)) {
            $value = [];
        }

        // Проверяем, есть ли агент в избранном
        if (in_array($agentID, $value)) {
            // Если агент уже в избранном, удаляем его
            $key = array_search($agentID, $value);
            if ($key !== false) {
                unset($value[$key]);
            }
        } else {
            // Если агента нет в избранном, добавляем его
            $value[] = $agentID;
        }

        // Сохраняем обновленный список избранных агентов
        CUserOptions::SetOption($category, $name, $value);

        // Формируем ответ
        $result['action'] = 'success';
        $result['starred'] = in_array($agentID, $value);
